...
export async function getAccessToken(env: Env, scope: string): Promise<string> {
  let current = tokenCache.get(scope)

  while (current) {
    const cachedToken = await current
    if (cachedToken.expiresAt > Date.now()) return cachedToken.accessToken

    // 만료됨 — await 사이에 다른 동시 호출이 이미 갱신을 시작했을 수 있으므로
    // 캐시를 재확인한다. 우리가 마지막으로 본 항목이 여전히 그대로일 때만
    // 우리가 갱신을 맡고, 아니면 그 새 항목을 기다린다(단일 비행 유지).
    const latest = tokenCache.get(scope)
  ...
  fresh.catch(() => {
    if (tokenCache.get(scope) === fresh) tokenCache.delete(scope)
  })
...
async function issueAccessToken(env: Env, scope: string): Promise<CachedToken> {
  ...
  if (typeof body.access_token !== 'string' || !body.access_token) {
    throw new Error('구글 토큰 응답에 access_token이 없습니다.')
  }
  if (typeof body.expires_in !== 'number' || !Number.isFinite(body.expires_in)) {
    throw new Error('구글 토큰 응답에 expires_in이 없습니다.')
  }
...
function parseServiceAccountKey(env: Env): ServiceAccountKey {
  ...
  if (typeof parsed !== 'object' || parsed === null) {
    throw new Error('GOOGLE_SERVICE_ACCOUNT_KEY가 JSON 객체가 아닙니다.')
  }

  const { client_email, private_key } = parsed as Partial<ServiceAccountKey>
...
function pemToArrayBuffer(pem: string): ArrayBuffer {
  ...
  try {
    ...
    throw new Error(`private_key를 base64로 디코드하지 못했습니다 (PEM 형식을 확인하세요): ${(e as Error).message}`)
  }
}
