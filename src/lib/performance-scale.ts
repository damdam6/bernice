...
// 레이더·랭킹 미니바가 같은 scale 인스턴스를 공유해 두 차트의 값이 정의상 일치한다.
// (종목 추이 차트는 이슈 #172로 원값 축으로 전환돼 이 함수를 쓰지 않는다 — 단위가 다른 다종목을
//  한 도형에 겹치는 레이더와, 행에 원값이 병기되는 랭킹 바만 정규화가 필요하다.)
//
// 규칙:
...
//
// 목표 대비 표시에도 같은 함수를 쓴다 — 목표가 관측 범위 밖이면 0/1로 클램프된다(전원 미달/
// 전원 초과의 자연스러운 표현). 범위 스케일 종목은 새 기록이 min/max를 갱신하면 과거 값의
// 정규화 값도 함께 움직인다 — 절대 성능이 아니라 팀 내 상대 성능 표현이다(§07).
