...
describe('TimeScoreInput', () => {
  it('분·초 값을 각 필드에 표시한다', () => {
    render(<TimeScoreInput label="드리블셔틀런" minutes="1" seconds="12" onChange={() => {}} />)
  ...
  it('분 필드를 바꾸면 seconds는 유지한 채 onChange를 호출한다', () => {
    ...
    render(<TimeScoreInput label="드리블셔틀런" minutes="1" seconds="12" onChange={onChange} />)

    fireEvent.change(screen.getByLabelText('드리블셔틀런 분'), { target: { value: '2' } })
  ...
  it('숫자가 아닌 문자는 걸러낸다', () => {
    const onChange = vi.fn()
    render(<TimeScoreInput label="드리블셔틀런" minutes="" seconds="" onChange={onChange} />)

    fireEvent.change(screen.getByLabelText('드리블셔틀런 초'), { target: { value: 'a5b' } })

    expect(onChange).toHaveBeenCalledWith({ minutes: '', seconds: '5' })
  })

  it('error가 있으면 alert로 노출한다', () => {
    render(<TimeScoreInput label="드리블셔틀런" minutes="1" seconds="75" onChange={() => {}} error="초는 0–59만 입력할 수 있어요" />)
